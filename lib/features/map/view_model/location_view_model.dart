...
class LocationViewModel extends ChangeNotifier {
  ...
  Future<void> loadCurrentLocation() async {
    ...
    _getLocationName(currentPosition.latitude, currentPosition.longitude);

    notifyListeners();
  }

  Future<String> _getLocationName(latitude,longitude) async {
    try {
      List<Placemark> placemarks = await placemarkFromCoordinates(latitude, longitude);
      if (placemarks.isNotEmpty) {
        Placemark place = placemarks.first;
        String address = '${place.street}, ${place.locality}';
        return _locationName = address;
      } else {
        return _locationName = 'Unknown place';
      }
    } catch (e) {
      debugPrint("Error fetching address: $e");
      return _locationName = 'Error!';
    }
  }
  ...
  Future<void> navigateToCurrentLocation({
    ...
    await _getLocationName(_currentLocation?.latitude, _currentLocation?.longitude);
